...
export default function Salidas() {
  ...
  function validate() {
    ...
    if (!form.cliente?.trim())   e.cliente = 'El cliente es obligatorio para las salidas';
  ...
  return (
    <div>
      <div className="page-header">
        <div>
          <h1 className="page-title">Salidas de Stock</h1>
          <p className="page-subtitle">{salidas.length} salidas registradas</p>
        </div>
        ...
        <table className="table">
          ...
          <tbody>
            {filtered.length === 0 ? (
              <tr><td colSpan={6}>
                <EmptyState
                  message={search ? 'No se encontraron salidas con ese criterio' : 'Sin salidas registradas'}
              ...
              <tr key={m.id}>
                ...
                <td className="text-right font-bold text-primary">
                  -{formatNumber(m.cantidad)} <span className="text-gray-400 font-normal text-sm">{m.unidad_medida}</span>
                </td>
                <td>
                  <span className="font-medium text-gray-700">{m.cliente || <span className="text-gray-300">—</span>}</span>
                </td>
                <td className="text-gray-500 text-sm max-w-xs truncate">{m.notas || <span className="text-gray-300">—</span>}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      >
        ...
        <div className="form-group">
          <label className="label">Cliente <span className="text-primary">*</span></label>
          <input
